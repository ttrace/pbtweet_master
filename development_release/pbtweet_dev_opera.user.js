...
function pb_init()
{
	...
	//information panel
	pb_version = "v1.5 Opera dev 0073";
			...
			for(var i = 0; i < update_node_number; i++ ){
				...
				if( insert_point_id < updated_entry_id )
				{
					...
					if(insert_point.id != "timeline"){
						...
						//updated_entry.style.oTransition = "opacity 0.35s cubic-bezier(0.4, 0.0, 0.6, 0.4), margin-top 0.2s ease-in";
						//updated_entry.style.oTransition = "opacity 0.35s cubic-bezier(0.4, 0.0, 0.6, 0.4);";
...
//	var snip_pattern = /http\:\/\/bit\.ly\/.+|http\:\/\/is\.gd\/.+|http\:\/\/twurl\.nl\/.+|http\:\/\/tinyurl\.com\/.+/;
	...
	else if( url_alias.match(bitly_pattern) )
	{
		...
		addClass(target.getElementsByTagName('span')[0],'loading');
//		BitlyClient.expand( url_alias , 'expandResponse');
...
function kick_animation(target)
{
	...
	if(target.parentNode.nextSibling)
	{
		...
		else if((target.parentNode.lastChild.offsetTop < (window.scrollY + window.innerHeight + 200)) && ((window.scrollY - 100) < target.parentNode.lastChild.offsetTop) )
		{
			...
			//target.style.oTransition = "opacity 0.45s cubic-bezier(0.4, 0.0, 0.6, 0.4), margin-top 0.35s ease-out";
			var conv_script = document.createElement('script');
...
function kick_animation_on_top(e)
{
	...
	{	// User can see animation
		//target.style.oTransition = "opacity 0.3s ease-in, margin-top 0.2s ease-in";
		var conv_script = document.createElement('script');
		...
		target.appendChild(conv_script);
		setTimeout(function(){target.style.opacity = "1";target.style.oTransition = "";target.style.marginTop = "0px";removeClass(target, "animate");pbtweet_main([target]);}, 1500);
	}
}
...
var pbtweet_css = document.createElement( 'link' );
	pbtweet_css.setAttribute( 'tyle' , 'text/css' );
	pbtweet_css.setAttribute( 'rel' , 'stylesheet' );
	pbtweet_css.setAttribute( 'href' , 'http://web.me.com/t_trace/pbtweet/pbtweet_opera.css' ); 
	document.getElementsByTagName( 'head' )[0].appendChild( pbtweet_css );
}
